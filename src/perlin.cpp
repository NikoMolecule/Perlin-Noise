...
float perlinNoise2(float x, float y) {
  ...
  Vector2 DirectionTopRight = {x - 1.0f, y};
  Vector2 DirectionTopLeft = {x, y};
  Vector2 DirectionBottomRight = {x - 1.0f, y - 1.0f};
  Vector2 DirectionBottomLeft = {x, y - 1.0f};
...
void PerlinNoise::genOff(int seed) {
  srand(seed);
...
float PerlinNoise::Get2d(float x, float y, float scale) {
  float total = 0, frequency = 1, amplitude = 1;

  for (int i = 0; i < octaves; i++) {
    total += perlinNoise2((x + offsets[i].x) / scale * frequency,
                          (y + offsets[i].y) / scale * frequency) *
             amplitude;
